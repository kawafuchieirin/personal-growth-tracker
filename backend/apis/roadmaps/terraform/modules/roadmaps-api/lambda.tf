# Lambda Function
resource "aws_lambda_function" "roadmaps" {
  ...
  environment {
    variables = {
      ROADMAPS_TABLE_NAME = var.dynamodb_table
...
resource "aws_lambda_permission" "api_gateway" {
  ...
  function_name = aws_lambda_function.roadmaps.function_name
  principal     = "apigateway.amazonaws.com"
  source_arn    = "${aws_apigatewayv2_api.roadmaps.execution_arn}/*/*"
}
